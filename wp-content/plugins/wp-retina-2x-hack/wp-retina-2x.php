...
function wr2x_picture_rewrite( $buffer ) {
	...
	foreach( $html->find( 'img' ) as $element ) {
		...
		if ( $parent->tag == "picture" ) {
			...
			$potential_retina = wr2x_get_retina( $element->src );
...
function wr2x_log( $data ) {
	if ( wr2x_is_debug() ) {
		$fh = fopen( trailingslashit( WP_PLUGIN_DIR ) . 'wp-retina-2x-hack/wp-retina-2x.log', 'a' );
...
function wr2x_get_retina_old( $file ) {
...
function get_s3objects_filepath() {
	$wr2xdir = explode("/", dirname( __FILE__ ) );
	$wr2xfolder = $wr2xdir[count($wr2xdir) - 1];
	$plugindir = str_replace($wr2xfolder, "", dirname(__FILE__));
	return $plugindir . 's3tools/s3objects.txt';
}

function s3_file_exists( $file ) {
	$uri = parse_url( $file );
	if( exec( 'grep ' . escapeshellarg( substr( $uri['path'], 1 ) ) . ' ' . get_s3objects_filepath() ) ) {
		return true;
	} else {
		return false;
	}
